...
import { bmrTrackerService } from '@/services'
import type { BMRTrackerUploadResult } from '@/services'
import type { BMRTrackerRecord } from '@/types'

const PAGE_SIZE = 50

type DraftRow = Partial<BMRTrackerRecord>

const BLANK_DRAFT: DraftRow = {
  date: null, client_name: '', product: '', batch_no: '',
}
...
function RowEditor({
  ...
  const set = (field: keyof BMRTrackerRecord) => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <tr className="bg-mustard/5">
      ...
      <td className="px-2 py-2"><input aria-label="Product" className={inputCls} value={draft.product ?? ''} onChange={set('product')} /></td>
      <td className="px-2 py-2"><input aria-label="Batch no" className={inputCls} value={draft.batch_no ?? ''} onChange={set('batch_no')} /></td>
      <td className="px-2 py-2 text-xs text-black/40 dark:text-slate-500 align-top">Save row to attach BMR</td>
...
function ImportSection({ onImported }: { onImported: () => void }) {
  ...
  const [result, setResult] = useState<BMRTrackerUploadResult | null>(null)
  ...
  const handleUpload = async () => {
    ...
    setError('')
    try {
      const res = await bmrTrackerService.bulkUpload(file)
      setResult(res)
  ...
  return (
    <details className="border border-black/10 dark:border-white/10 rounded-lg">
      <summary className="cursor-pointer px-4 py-3 font-medium text-sm text-black dark:text-white select-none">
        Import old BMR tracker data from Excel
      </summary>
      <div className="px-4 pb-4 pt-1 space-y-3">
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <button onClick={() => bmrTrackerService.downloadTemplate()} className="btn-secondary text-sm">
            ↓ Download Template
          </button>
        </div>
...
export default function BMRTracker() {
  const searchId = useId()
  const user = useAuthStore((s) => s.user)
  const isAdmin = user?.role === 'admin'

  const [rows, setRows] = useState<BMRTrackerRecord[]>([])
  ...
  const fetchRows = (q = search, p = page) => {
    setLoading(true)
    bmrTrackerService.list({ ...(q ? { q } : {}), page: p, page_size: PAGE_SIZE })
      .then((data) => {
      ...
      .catch(() => setError('Failed to load BMR tracker records.'))
  ...
  const handleAddSave = async () => {
    ...
    setSaving(true)
    try {
      await bmrTrackerService.create(addDraft)
      setAdding(false)
  ...
  const startEdit = (row: BMRTrackerRecord) => {
    setEditingId(row.id)
    setEditDraft({ ...row })
  }
  ...
  const handleEditSave = async () => {
    ...
    setError('')
    try {
      const updated = await bmrTrackerService.update(editingId, editDraft)
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Delete this BMR tracker record? This cannot be undone.')) return
    try {
      await bmrTrackerService.remove(id)
  ...
  const headers = ['Sr No.', 'Date', 'Client Name', 'Product', 'Batch No', 'BMR (Attachment)', 'Actions']

  return (
    <Layout title="BMR Tracker">
      <div className="space-y-6">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <h1 className="text-2xl font-bold text-black dark:text-white">BMR Tracker</h1>
          <button
            type="button"
        ...
        <form onSubmit={handleSearch} role="search" aria-label="Search BMR tracker records">
          <div className="flex gap-2">
            <label htmlFor={searchId} className="sr-only">Search by client, product, or batch number</label>
            <input
              id={searchId}
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search by client, product, or batch number…"
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto rounded border border-black/10 dark:border-white/10">
            <table className="w-full text-sm" aria-label="BMR tracker">
              <thead>
              ...
              <tbody className="divide-y divide-black/5 dark:divide-white/5">
                ...
                {rows.length === 0 && !adding ? (
                  <tr><td colSpan={headers.length} className="px-3 py-6 text-center text-black/50 dark:text-slate-400">No BMR tracker records found.</td></tr>
                ) : (
                  rows.map((r, i) =>
                    editingId === r.id ? (
                      ...
                      <tr key={r.id} className="hover:bg-black/2 dark:hover:bg-white/2">
                        ...
                        <td className="px-3 py-2 text-black/70 dark:text-slate-300 align-top">{r.product || '—'}</td>
                        <td className="px-3 py-2 font-mono text-black dark:text-white align-top">{r.batch_no || '—'}</td>
                        <td className="px-3 py-2 align-top">
                          <DocumentsCell
                            entityId={r.id}
                            isAdmin={isAdmin}
                            listFiles={bmrTrackerService.listFiles}
                            uploadFile={bmrTrackerService.uploadFile}
                            deleteFile={bmrTrackerService.deleteFile}
                          />
                        </td>
        ...
        {count > PAGE_SIZE && (
          <div className="flex items-center justify-between text-sm">
            <span className="text-black/60 dark:text-slate-400" role="status" aria-live="polite">
              Page {page} of {totalPages} · {count} BMR tracker records
            </span>
            <div className="flex gap-2">
