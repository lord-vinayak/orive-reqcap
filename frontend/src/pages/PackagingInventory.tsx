...
import { packagingRecordService } from '@/services'
import type { PackagingUploadResult } from '@/services'
import type { PackagingRecord } from '@/types'

const PAGE_SIZE = 50

type DraftRow = Partial<PackagingRecord>
...
function RowEditor({
  ...
  const set = (field: keyof PackagingRecord) => (e: React.ChangeEvent<HTMLInputElement>) => {
...
function ImportSection({ onImported }: { onImported: () => void }) {
  ...
  const [result, setResult] = useState<PackagingUploadResult | null>(null)
  ...
  const handleUpload = async () => {
    ...
    setError('')
    try {
      const res = await packagingRecordService.bulkUpload(file)
      setResult(res)
  ...
  return (
    <details className="border border-black/10 dark:border-white/10 rounded-lg">
      <summary className="cursor-pointer px-4 py-3 font-medium text-sm text-black dark:text-white select-none">
        Import old packaging data from Excel
      </summary>
      <div className="px-4 pb-4 pt-1 space-y-3">
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <button onClick={() => packagingRecordService.downloadTemplate()} className="btn-secondary text-sm">
            ↓ Download Template
          </button>
        </div>
              ...
              <div>
                ...
                <ul className="list-disc list-inside text-black/60 dark:text-slate-300">
                  {result.created.filter((r) => r.warning).map((r) => (
                    <li key={r.row}>Row {r.row} ({r.product_name}): {r.warning}</li>
                  ))}
                </ul>
              </div>
            )}
...
export default function PackagingInventory() {
  ...
  const fetchRows = (q = search, p = page) => {
    setLoading(true)
    packagingRecordService.list({ ...(q ? { q } : {}), page: p, page_size: PAGE_SIZE })
      .then((data) => {
      ...
      .catch(() => setError('Failed to load packaging records.'))
  ...
  const handleAddSave = async () => {
    ...
    setSaving(true)
    try {
      await packagingRecordService.create(addDraft)
      setAdding(false)
  ...
  const startEdit = (row: PackagingRecord) => {
    setEditingId(row.id)
    setEditDraft({ ...row })
  }
  ...
  const handleEditSave = async () => {
    ...
    setError('')
    try {
      const updated = await packagingRecordService.update(editingId, editDraft)
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Delete this packaging record? This cannot be undone.')) return
    try {
      await packagingRecordService.remove(id)
  ...
  const headers = [
    'Count', 'Date', 'Category', 'Client Name', 'Vendor Name', 'Product Name',
    'No. Of Ordered by Client', 'Quantity Sent', 'Used in Batch', 'Quantity Remained',
    'Price Per Unit', 'Damaged / Missing', 'Comment', 'Product Type', 'Actions',
  ]

  return (
    <Layout title="Packaging Inventory">
      <div className="space-y-6">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <h1 className="text-2xl font-bold text-black dark:text-white">Packaging Inventory</h1>
          <button
            type="button"
        ...
        <form onSubmit={handleSearch} role="search" aria-label="Search packaging records">
          <div className="flex gap-2">
            <label htmlFor={searchId} className="sr-only">Search by client, vendor, product, category, or product type</label>
            <input
              id={searchId}
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search by client, vendor, product, category, or product type…"
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto rounded border border-black/10 dark:border-white/10">
            <table className="w-full text-sm" aria-label="Packaging inventory">
              <thead>
              ...
              <tbody className="divide-y divide-black/5 dark:divide-white/5">
                ...
                {rows.length === 0 && !adding ? (
                  <tr><td colSpan={headers.length} className="px-3 py-6 text-center text-black/50 dark:text-slate-400">No packaging records found.</td></tr>
                ) : (
                  rows.map((r, i) =>
                    editingId === r.id ? (
        ...
        {count > PAGE_SIZE && (
          <div className="flex items-center justify-between text-sm">
            <span className="text-black/60 dark:text-slate-400" role="status" aria-live="polite">
              Page {page} of {totalPages} · {count} packaging records
            </span>
            <div className="flex gap-2">
