...
import { FilePlus2, FileSearch, FolderKanban, LayoutDashboard, ClipboardList, PackageSearch, FlaskConical, Box, Users } from "lucide-react";
  ...
  return (
    <Layout title="Home">
      <div className="max-w-4xl mx-auto pt-8">
        ...
        {/* Batch Register + Ingredient Inventory + Packaging Inventory + Packaging Client List — side by side */}
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
          ...
          <div
            onClick={() => navigate("/packaging-clients")}
            onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); navigate("/packaging-clients"); } }}
            role="button"
            tabIndex={0}
            className="card text-left hover:border-mustard transition-colors group cursor-pointer flex items-center gap-5 focus:outline-none focus-visible:ring-2 focus-visible:ring-mustard"
            aria-label="Go to Packaging Client List — track packaging vendors, costs, and POC contacts per client"
          >
            <div className="w-12 h-12 bg-mustard-50 dark:bg-mustard-100 rounded flex items-center justify-center shrink-0 group-hover:bg-mustard transition-colors" aria-hidden="true">
              <span className="text-2xl text-mustard-700"><Users /></span>
            </div>
            <div>
              <div className="text-lg font-semibold text-black dark:text-slate-100 mb-0.5" aria-hidden="true">Packaging Client List</div>
              <div className="text-sm text-black/60 dark:text-slate-300" aria-hidden="true">
                Track packaging vendors, costs, and POC contacts per client.
              </div>
            </div>
          </div>
        </div>
