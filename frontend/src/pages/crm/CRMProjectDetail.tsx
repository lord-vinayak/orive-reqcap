    ...
    <section className="mt-8 bg-white dark:bg-slate-800 rounded-xl border border-black/10 dark:border-white/10 p-6">
          ...
          <tbody>
            {invoices.map((inv) => (
              <tr key={inv.id} className="border-b border-black/5 dark:border-white/5 last:border-0">
                ...
                <td className="py-2">
                  {inv.status === 'draft' ? (
                    <span className="inline-block text-xs font-medium px-1.5 py-0.5 rounded bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400">
                      Draft
                    </span>
                  ) : (
                ...
                <td className="py-2 text-xs text-gray-600 dark:text-gray-300">{inv.comment || '—'}</td>
                ...
                <td className="py-2">
                  <div className="flex items-center gap-3">
                    {inv.status === 'draft' && (
                      <button
                        type="button"
                        onClick={() => setDraftToEdit(inv)}
                        className="text-xs text-mustard underline-offset-2 hover:underline"
                        aria-label={`Continue editing draft invoice ${inv.invoice_number}`}
                      >
                        continue
                      </button>
                    )}
      ...
      {(modalOpen || draftToEdit) && (
        <GenerateInvoiceModal
          ...
          billingInfo={billingInfo}
          draftInvoice={draftToEdit}
          onClose={() => { setModalOpen(false); setDraftToEdit(null) }}
          onDone={(inv) => {
            upsertInvoice(inv)
            setModalOpen(false)
            setDraftToEdit(null)
          }}
          onDraftSaved={upsertInvoice}
        />
      )}
    </section>
  )
